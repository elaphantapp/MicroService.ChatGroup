...
namespace chatgroup {
    ...
    void DatabaseProxy::updateMemberInfo(const std::string& friendid,
        ...
        if (member_info.get() != nullptr) {
            ...
            member_info->mStatus = status == ConnectionStatus_OnLine ? 0 : 1;
            t_strSql = "update member_table set NickName='"+tmp_nickname+"'";
            t_strSql += ",Status="+std::to_string(member_info->mStatus);
            t_strSql += ",MsgTimeStamp="+std::to_string(member_info->mMsgTimeStamp);
            t_strSql += " where Friendid='"+member_info->mFriendid+"';";
            member_info->UnLock();

        } else {
            member_info = std::make_shared<chatgroup::MemberInfo>(
                    friendid, nickname, status, time_stamp);
            mMemberList->push_back(member_info);
            t_strSql = "insert into member_table values(NULL,'"+member_info->mFriendid+"'";
            t_strSql += ",'"+member_info->mNickName+"'";
    ...
    DatabaseProxy::getMemberInfo(const std::string& friendid) {
        MUTEX_LOCKER locker_sync_data(_SyncedMemberList);
    ...
    bool DatabaseProxy::deleteRemovedListMember(const std::string& friendid) {
        MUTEX_LOCKER locker_sync_data(_SyncedMemberList);
    ...
    std::shared_ptr<MemberInfo>
    DatabaseProxy::getBlockMemberInfo(const std::string& friendid) {
    ...
    std::string DatabaseProxy::getGroupNickName() {
        return mNickName;
    }

    void DatabaseProxy::updateGroupNickName(const std::string& nick_name) {
        MUTEX_LOCKER locker_sync_data(_SyncedGroupInfo);
        std::string t_strSql = "";
        char *errMsg = NULL;
        if (mNickName.empty()) {
            t_strSql = "insert into group_info_table values(NULL,'"+nick_name+"');";
        } else {
            t_strSql = "update group_info_table set NickName='"+nick_name+"' where id=1;";
        }
    ...
    void DatabaseProxy::addMessage(const std::string& friend_id,
                                   const std::string& message, std::time_t send_time) {
        MUTEX_LOCKER locker_sync_data(_SyncedMessageList);
        char *errMsg = NULL;
        std::string t_strSql;
        t_strSql = "insert into message_table values(NULL,'"+friend_id+"'";
        t_strSql += ",'"+message+"'";
    ...
    DatabaseProxy::getMessages(const std::string& friend_id, std::time_t send_time, int max_limit) {
        MUTEX_LOCKER locker_sync_data(_SyncedMessageList);
        //查询一条记录
        ...
        int ncolumn;
        std::string t_strSql;
        t_strSql = "select * from message_table where FriendId!='"+friend_id+"'";
        ...
        if (nrow != 0 && ncolumn != 0) {     //有查询结果,不包含表头所占行数
            for (int i = nrow; i >=1; i--) {        // 第0行为数据表头
                ...
                message_info->mFriendid = std::string(azResult[4*i + 1]);
                message_info->mMsg = std::string(azResult[4*i + 2]);
    ...
    bool DatabaseProxy::inRemovedList(const std::string& friendid) {
        MUTEX_LOCKER locker_sync_data(_SyncedMemberList);
        for (int i = 0; i < mRemovedMemberList->size(); i++) {
            std::shared_ptr<MemberInfo> member_info = mRemovedMemberList->at(i);
            if (member_info.get() == nullptr) {
    ...
    bool DatabaseProxy::removeMember(const std::string& friendid) {
        MUTEX_LOCKER locker_sync_data(_SyncedMemberList);
        for (int i = 0; i < mMemberList->size(); i++) {
            std::shared_ptr<MemberInfo> member_info = mMemberList->at(i);
            if (member_info.get() == nullptr) {
    ...
    void DatabaseProxy::addBlockMember(std::shared_ptr<MemberInfo> memberinfo){
        ...
        t_strSql = "insert into block_member_table values(NULL,'"+memberinfo->mFriendid+"');";
    ...
    bool DatabaseProxy::removeBlockMember(const std::string& friendid) {
        MUTEX_LOCKER locker_sync_data(_SyncedBlockMemberList);
        for (int i = 0; i < mBlockMemberList->size(); i++) {
            std::shared_ptr<MemberInfo> member_info = mBlockMemberList->at(i);
            if (member_info.get() == nullptr) {
    ...
    void DatabaseProxy::syncGroupInfo() {
        ...
        if (nrow != 0 && ncolumn != 0) {     //有查询结果,不包含表头所占行数
            for (int i = 1; i <= nrow; i++) {        // 第0行为数据表头
                mNickName =  std::string(azResult[2*i + 1]);
        ...
        if (nrow != 0 && ncolumn != 0) {     //有查询结果,不包含表头所占行数
            for (int i = 1; i <= nrow; i++) {        // 第0行为数据表头
                mMemberList->push_back(std::make_shared<MemberInfo>(
                        std::string(azResult[5*i + 1]),
                        std::string(azResult[5*i + 2]),
                        1,//初始化时为offline
                        atol(azResult[5*i + 4])));
            }
        }
    ...
    bool DatabaseProxy::startDb(const char *data_dir) {
        std::string strConn = std::string(data_dir) + "/chatgroup.db";
        char *errMsg;
